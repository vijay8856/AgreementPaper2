...
//             <Text>Total Spend: {totalSpend}</Text>
//           </View>
...
//             />
//           ) : (
//             <Text style={{ textAlign: 'center', color: 'gray' }}>Loading chart...</Text>
//           )}
              ...
              <TouchableOpacity onPress={() => navigation.navigate("MyProfile")}>
                {userData?.profile_pic ? (
                  <Image
                    ...
                    onError={(e) => console.log("Profile error →", e.nativeEvent.error)}
                  />
                ) : (
                  <Icon name="account-circle" size={28} color="#fff" />
                )}
  ...
  const StatusCard = ({ title, sections, loading = false }: any) => (
    <View style={styles.statusCard}>
      ...
      {loading ? (
        ...
        <View style={styles.statusSections}>
          {sections.map((section: { value: any; label: any; }, index: React.Key | null | undefined) => (
            <View key={index} style={styles.statusSection}>
              <Text style={styles.sectionValue}>{section.value}</Text>
              <Text style={styles.sectionLabel}>{section.label}</Text>
            </View>
      ...
      >
        ...
        <View style={styles.chartContainer}>
          ...
          {loadingChart ? (
            ...
            <>
              <Text style={styles.totalSpendText}>Total: ${totalSpend.toLocaleString()}</Text>
              <LineChart
                data={chartData}
        ...
        <View style={styles.chartContainer}>
          ...
          {loadingChart ? (
            ...
            <Text style={styles.noDataText}>No contract status data available</Text>
          )}
        </View>
