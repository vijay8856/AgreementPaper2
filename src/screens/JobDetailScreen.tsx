...
const JobDetailScreen = ({ route, navigation }: any) => {
  ...
  const handleSubmitApplication = async () => {
    // Basic validations
    if (!firstName.trim()) return Alert.alert("Validation", "Please enter first name");
    if (!lastName.trim()) return Alert.alert("Validation", "Please enter last name");
    if (!email.trim() || !validateEmail(email)) return Alert.alert("Validation", "Please enter valid email");
    if (!contact.trim()) return Alert.alert("Validation", "Please enter contact number");
    if (!selectedCountry) return Alert.alert("Validation", "Please select a country");
    if (!selectedState) return Alert.alert("Validation", "Please select a state");
    if (!locationCity.trim()) return Alert.alert("Validation", "Please enter location/city");
  ...
  if (loading || !job) {
    return (
      <View style={styles.loaderContainer}>
        ...
        <Text style={{ marginTop: 10, color: "#00007B" }}>Loading Job Details...</Text>
      </View>
    );
  }
  ...
  return (
    <>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        {/* Salary */}
        {pay_rate ? <Text style={styles.salary}>INR {pay_rate}/day</Text> : null}
        ...
        <Text style={styles.meta}>📍 {company_city}, {company_state}</Text>
